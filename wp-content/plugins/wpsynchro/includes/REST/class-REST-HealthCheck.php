...
class HealthCheck
{
    ...
    public function service($request)
    {
        ...
        if ($accesskey_okay && $permalinks_okay && $env_okay && $licenseokay) {
            ...
            $initiate_url = get_home_url(null, "wp-json/wpsynchro/v1/initiate/");
            $args = array(
                'method' => 'POST',
                'timeout' => 30,
                'redirection' => 0,
            ...
            if (is_wp_error($initiate_response)) {
                ...
                if (wp_remote_retrieve_response_code($initiate_response) == 200) {
                    ...
                    error_log(print_r($initiate_response, true));
                }
            }
        ...
        if (\WPSynchro\WPSynchro::isPremiumVersion()) {
            $paths_check = array(
                ...
                // Absolut directory of WordPress root folder, that can be different than document_root
                ABSPATH,
            ...
            foreach ($paths_check as $path) {
                $path = realpath($path);
                if (!is_writable($path)) {
                    $healthcheck->warnings[] = sprintf(__("Possible path that WP Synchro files synchronization can use is not writable- Path: %s", "wpsynchro"), $path);
                }
            }
        }
